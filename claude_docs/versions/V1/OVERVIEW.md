...
> 最后更新：2026-09-10（随 V1.10.1 发布）

## 能力快照

- **定位**：可通过 docker compose 独立部署的后端微服务，对外提供两个异步 Agent API（评论价值初筛 / 账号画像精筛），同时保留 V0 的 8000 测试链路。
- **对外契约**：`POST /api/v1/comment-screening`、`POST /api/v1/profile-analysis` 提交作业，`GET /api/v1/jobs/{job_id}` 轮询结果；静态 API Key 认证（`Authorization: Bearer`）；`GET /health` 探活。V1.9.1 起新增黑名单管理 API（`GET/POST/DELETE /api/v1/blacklist`，同步分页列表/批量幂等入库/逐条删除，复用 API Key 鉴权，见下）。对接文档：`docs/DriveIntent-V1-API对接文档.md`（当前 1.4 版）。V1.7.3 起 Agent2 等级多对一映射：H/A→high、B→medium、C→low，分数区间随之调整（见对接文档等级表）。V1.8.0 起 Agent2 结果新增 `intent_models`（意向车型）/ `intent_model_category`（四档分类，标准可配）两字段；V1.8.2 起 `intent_model_category` 对外返回中文正式内容（配置 `label`：东风猛士系列/越野车/25-30万SUV/其他），库内仍存码值 A/B/C/D 供内部统计。V1.9.0 起命中黑名单的账号对外 `has_value=false` 且 `analysis` 文本含"黑名单"字样（其余等级/评分字段为 null，详见下）。V1.9.2 起 Agent2 结果新增 `is_blacklisted` / `blacklist_type` / `blacklist_reason` 三字段（确认与疑似黑名单统一口径，见下）。V1.10.0 起 Agent2 结果新增 `our_model_intent_level`（用户对我方在售车型的购车意向：高/中/低）与 `recommend_our_model`（最适合推荐的我方在售车型名，取自 `config/our_models.json`）两字段。
- **核心能力现状（V1.6 后）**：
  ...
  - **Agent2（账号精筛）**：定级处理前先做零 LLM 的黑名单匹配（V1.9.0，仅 API 路径）：`account_douyin_id` 命中 `blacklisted_user` 表即直接输出"黑名单用户"（C / is_valid_lead=False / filter_category=blacklisted），跳过识图/无效用户过滤/定级/复核/润色全部 LLM 节点，对外 `has_value=false`，`is_blacklisted=true` / `blacklist_type="confirmed"`（V1.9.2）；黑名单经管理页 `/blacklist` 维护。定级前先过"无效用户过滤"节点（独立 LLM 调用，V1.6）——已购无新购计划/推广他人/仅替他人问询/疑似营销号/汽车从业者/其他六类命中直接定 C 不进定级，fail-open 放行；V1.9.2 起该节点同时识别**疑似黑名单账号**（`blacklist_type="营销号"` / `"虚假账号"`，规则见 V1.9.2 设计），命中优先于六类过滤类别直接定 C、跳过定级/复核/润色，且**两路径（API 与 V0）均生效**（跟随共用过滤节点）；对外透出 `is_blacklisted` / `blacklist_type` / `blacklist_reason` 三字段，供前端拉黑异常账号时参考（判定基于 LLM，建议人工复核后再拉黑）。账号级两标签 + H/A/B/C 定级三阶段流水线（V1.6.2，V1.8.0 重构阶段二）——评论基线（多条同质评论累积强化已并入基线）→ 意向车型识别与分类（V1.8.0：只识别归档、**不调整评级**；识别有购买意向的车型 `intent_models`，按可配置四档标准输出 `intent_model_category`，标准配置 `config/intent_categories.json`；V1.2.1 引入的匹配调级与 V1.7.4 的对比基准约束随之退役，如何用分类调整优先级交下游决定）→ 主页截图结构化画像有限上调（作用于基线评级）；阶段二（V1.10.0）另判定 `our_model_match`（our_model/similar/unrelated/unknown），由代码按既定规则对意向强度做只降不升的降级，产出对外 `our_model_intent_level`；`recommend_our_model` 由 LLM 从在售清单选定、代码校验后透出（不在清单内置 null）。该判定**不参与评级调整**。定级后接分级复核（V1.7.0）——初始 C 不审查不润色直接输出、初始 B 走普通模型审查（`user_lead_review`，V1.6.2/1.6.3 销售视角复核 + 单次至多改一级、C 为下限、fail-open）、初始 A/H 走高级模型审查（`user_lead_review_advanced`，逐段核验五段论证与推理链，模型路由到 `LLM_MODEL_ADVANCED` 并强制深度思考）；初始 B 经普通审查 upgrade 到 A/H 时追加一次高级终审。复核改级时一并修订对外叙述（V1.6.3）——按段标题锚点替换 `analysis_text` 第五段"总体评价"与 `lead_summary`，前四段事实陈述逐字保留，锚点缺失退化为文末追加。V1.8.4：定级 Prompt 收紧 `analysis_text` 第四段"主页画像与调整结论"的措辞——只陈述画像事实与是否触发上调，禁止输出等级性结论（如"最终评级维持/调整为 X 级"），等级结论统归第五段，消除"初评 B 经复核降级到 C"时第四段残留旧等级的矛盾。复核后统一接 analysis 润色节点 `user_analysis_polish`（V1.6.4，两路径同接，最终定级 C 跳过）——针对最终定级重写 analysis_text / lead_summary / profile_summary，清除英文字段泄漏、消除叙述与定级矛盾，只改文本不改级，fail-open 保留原文；复核节点同版起接入对外 API 路径（此前仅 V0 流水线有复核）。**等级对外映射（V1.7.3）**：H/A→high、B→medium、C→low，各内部等级保留原 base/floor（C 新增 45/40）；由于外部 code 已多对一，`api_job.lead_grades` 落每账号真实 HABC 供内部审计读取（`lead_results.py`），审计仍以 HABC 四档呈现，旧数据回退按 code 反推。
  - **配套能力**：视频语境分析注入初筛与用户证据包；主页截图识图（结构化画像 JSON）；我方在售车型配置（`our_models.json`）；意向车型分类标准配置（`intent_categories.json`，V1.8.0）；黑名单管理页 `/blacklist`（V1.9.0，批量录入/列表/逐条删除，与 `/audit` 同层、无额外鉴权）与对外黑名单管理 API `/api/v1/blacklist`（V1.9.1，同步分页列表/批量幂等入库/逐条删除，复用 API Key 鉴权，接口见对接文档 §5.5）。
  ...
  - **线索列表数据源（V1.10.1）**：`/leads` 与 CSV 导出改读 `lead_record` 汇总表（一行一账号，`api_job.result` 的物化派生表），作业终态时同事务写入，历史数据由 `scripts/backfill_lead_records.py` 回填。此前列表需在查询期展开 `result` 大列（平均 50 KB/作业、全量 346 MB）才能算分页游标，导致首页 8s、等级筛选 140s。切换判据是就绪标记文件 `data/lead_record_ready`（回填自检通过后写入；全新库由服务启动时自动写入），而非"汇总表有数据"——回填按时间升序写入而列表按时间倒序展示，按"有数据"切换会在回填中途静默只显示最旧的那批作业。未就绪时自动回退原 JSON 展开路径。详情页仍读 `api_job` 原始 JSON（单作业一次性读取，本就很快）。
...
| V1.9.0 | 2026-09-07 | 用户黑名单机制：Agent2 精筛定级前置零 LLM 黑名单匹配（account_douyin_id 命中即输出黑名单用户、跳过全部 LLM 节点）+ /blacklist 管理页（批量录入/列表/删除）；仅 API 路径，V0 未接入，对外契约不变 | [design](V1.9/design.md) / [plan](V1.9/plan.md) |
| V1.9.1 | 2026-09-08 | 用户黑名单对外增删查 API：`/api/v1/blacklist`（分页列表/关键字搜索/批量幂等入库/逐条删除），复用 API Key 鉴权，服务层 list_blacklist 扩展分页搜索（旧签名向后兼容）；分析接口零改动，纯增量 | [design](V1.9/v1.9.1-design.md) / [plan](V1.9/v1.9.1-plan.md) |
| V1.9.2 | 2026-09-08 | 疑似黑名单账号识别：`user_lead_filter` 节点（无效用户过滤，一次 LLM 调用）同时识别疑似营销号/疑似虚假账号，命中优先于过滤类别直接定 C、跳过定级/复核/润色（API 与 V0 两路径均生效）；对外新增 `is_blacklisted` / `blacklist_type` / `blacklist_reason` 三字段（确认黑名单统一口径 type="confirmed"）；过滤 Prompt 升 v1.9.2，契约纯增量 | [design](V1.9/v1.9.2-design.md) |
| V1.10.0 | 2026-09-09 | 我方在售车型购车意向分析：定级阶段二新增 `our_model_match` 语义判定，代码确定性降级产出对外 `our_model_intent_level`（高/中/低）与 `recommend_our_model`（在售车型名，代码校验）；Prompt 升 v1.10.0，契约纯增量 | [design](V1.10/design.md) / [plan](V1.10/plan.md) |
| V1.10.1 | 2026-09-10 | 补丁：管理页卡顿修复——补生产库缺失的 `ix_api_job_finished` / `ix_llm_call_created`（`create_all` 不给已存在表补索引，测试库全新建表故未暴露）并新增线索列表覆盖索引；新增 `lead_record` 汇总表把线索列表从"查询期展开 346 MB result 大列"改为普通列分页（等级筛选 140s→0.03s、首页 8.3s→0.12s，实测），审计页 10s+→1s 内；对外契约与页面行为不变 | [design](V1.10/v1.10.1-design.md) |
