...
- **核心能力现状（V1.6 后）**：
  ...
  - **配套能力**：视频语境分析注入初筛与用户证据包；主页截图识图（结构化画像 JSON）；我方在售车型配置（`our_models.json`）；意向车型分类标准配置（`intent_categories.json`，V1.8.0）；黑名单管理页 `/blacklist`（V1.9.0，批量录入/列表/逐条删除，与 `/audit` 同层、无额外鉴权）。
