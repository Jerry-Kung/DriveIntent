...
> **V1.9.0**：新增 `blacklisted_user` 表（用户黑名单），属"建新表"而非"为已有表补列"，由服务启动时 `init_db()` 的 `create_all` 自动创建，**无需执行任何迁移脚本**；重启服务即建表。黑名单数据经管理页 `/blacklist` 维护（批量录入/列表/逐条删除，管理面，与 `/audit` 同层、无额外鉴权）。

> **V1.10.1**：新增 `lead_record` 表（线索汇总表，`api_job.result` 的物化派生表），同样由 `create_all` 自动建出，**无需迁移脚本**。但**存量数据需要回填一次**，否则列表页会一直走慢的回退路径（见下）。
...
# 升级到 V1.10.1：补性能索引（api_job 完成时间 / 线索列表覆盖索引；
# llm_call_log 时间索引）。幂等，可在服务运行中执行
python scripts/add_perf_indexes.py
```

### 10.1 V1.10.1 存量数据回填（升级后必做一次）

管理页 `/leads` 与 `/audit` 卡顿的根因之一是**生产库缺索引**：模型里声明的 `ix_api_job_finished`、`ix_llm_call_created` 只在新表创建时生效，`create_all` 不会给已存在的表补索引，而测试库是全新建表故未暴露。补索引解决大部分延迟，剩下的线索列表延迟来自查询期展开 `result` 大列（平均 50 KB/作业），改由 `lead_record` 汇总表承担。

两步操作，均可在服务运行中执行，无需停机：

```bash
# 第一步：补索引（秒级；审计页与列表页排序立即可见效）
python scripts/add_perf_indexes.py

# 第二步：回填线索汇总表（全量约 7 千作业 / 26 万行，实测约 3 分钟）
python scripts/backfill_lead_records.py
```

回填脚本幂等（以 `(job_id, idx)` 为主键 upsert），中断后重跑不会产生重复行，也不会覆盖已写入的正确数据；`--limit N` 可先小批试跑，`--dry-run` 只统计不写库，`--chunk N` 控制单条 INSERT 的行数（默认 200）。**回填完成前列表页仍可用**：未就绪时服务自动回退原 JSON 展开路径（较慢但结果完整），因此回填与线上服务互不阻塞。全量实测 7,288 作业 / 262,246 行约 4 分钟。

**切换判据是就绪标记文件 `data/lead_record_ready`**，不是"汇总表有数据"。原因是回填按 `finished_at` 升序写入而列表按时间倒序展示：若按"有数据"就切换，回填中途失败会让列表**静默地**只显示最旧的那批作业，最新的线索恰好不见且无人察觉。标记由回填脚本在**完备性自检通过后**写入，脚本结束时会打印自检结果：

- 自检通过（仍有作业未回填数 = 0）→ 写入标记 → 列表立即切到快路径；
- 自检未通过（报"还有 N 个作业未回填"）→ 不写标记 → 列表继续走完整但较慢的回退路径，重跑脚本（幂等）即可补齐。

自检以**"还有没有未被回填的作业"**为判据（`NOT EXISTS`），而不是行数差：行数会被回填期间新完成的作业持续扰动（实测一次运行中漂移 178 行），按行数差判断会误报失败。

回填是长任务，连接可能被中间链路回收。脚本每次使用连接前会探活、失效即重建并重放当批；由于写入是幂等 upsert，重放安全，单次链路中断不会让整个回填失败（该情形在首次执行时真实发生过，表现为结尾报 `InterfaceError (0, '')` 而数据其实已写完）。**如果脚本中途报错，直接重跑即可**，并从自检输出确认结果，不必先清表。

标记与截图暂存区同在 `./data` 挂载点下（docker 与裸机部署共享），**不要手工创建该文件**：它等于声明"汇总表已完整"，若实际不完整会造成上述静默缺数据。全新部署无需回填——服务启动时检测到库内无历史精筛作业会自动写入标记。

回填后可用下面的命令交叉核对（脚本已自动执行同一核对，此处供人工复核）。判据是**第一个查询返回 0**——行数差会被运行期间新完成的作业扰动，只作参考：

```sql
-- 权威判据：还有多少作业未被回填，应为 0
SELECT COUNT(*) FROM api_job j
WHERE j.job_type='profile_analysis' AND j.result IS NOT NULL
  AND j.status IN ('success','partial')
  AND NOT EXISTS (SELECT 1 FROM lead_record r WHERE r.job_id = j.id);

-- 参考：行数口径（汇总表行数 vs JSON_LENGTH 之和，允许小幅漂移）
SELECT COUNT(*) FROM lead_record;
SELECT SUM(JSON_LENGTH(result, '$.results')) FROM api_job
WHERE job_type='profile_analysis' AND result IS NOT NULL
  AND status IN ('success','partial');
```

历史版本的迁移脚本（V1.4 补审计索引等）已在现有环境执行完毕并从仓库移除。全新部署无需执行任何迁移脚本，首次启动 `create_all` 会自动建出全部最新表结构，新建的 `lead_record` 也从空开始累积。
