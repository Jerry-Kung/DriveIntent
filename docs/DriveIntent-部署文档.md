...
回填脚本幂等（以 `(job_id, idx)` 为主键 upsert），中断后重跑不会产生重复行，也不会覆盖已写入的正确数据；`--limit N` 可先小批试跑，`--dry-run` 只统计不写库。**回填完成前列表页仍可用**：未就绪时服务自动回退原 JSON 展开路径（较慢但结果完整），因此回填与线上服务互不阻塞。
