...
| `intent_model_category` | String \| null | **V1.8.0** 意向车型分类：`"A"`/`"B"`/`"C"`/`"D"`（分类标准服务端可配，默认 A=东风猛士系列、B=越野车、C=25-30万SUV、D=其他或无意向车型）；未配置标准/处理失败时为 `null` |
...
2. **新增两个输出字段**（纯增量，既有字段与结构不变）：
   - `intent_models`：该账号有购买意向的车型列表（严格基于评论证据综合判定，
     提及≠意向；空数组=无意向车型）；
   - `intent_model_category`：意向车型四档分类 `"A"`/`"B"`/`"C"`/`"D"`
     （多款意向车型取最优档；分类标准服务端可配置，默认
     A=东风猛士系列、B=越野车、C=25-30万SUV、D=其他或无意向车型；
     未配置标准时为 `null`）。

对接方如不消费新字段可忽略，无需改动既有解析逻辑。
